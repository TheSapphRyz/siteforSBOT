...
| **img data** | **=**{{"t", 'pi'}, {'i', id}, {"np", np}, {"vp", vp}, {'p', m} } | {{'t', 'i'}, {'s', 1}} | id - айди сообщения, np - номер текущего пакета, vp - количество пакетов, m - байты изображения в base64 | m в бд должен быть в шифровке, а не BLOB |
...
| **doc data** | **=**{{'t', 'di'}, {'i', id}, {"np", np}, {"vp", vp}, {'p', m} } | {{'t', 'd'}, {'i', did}} | id - айди сообщения, np - номер текущего пакета, vp - количество пакетов, m - байты документа в base64 | сервер пересылает это сообщение без b, но с did |
