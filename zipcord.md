...
| **img msg** | **=**{{"t", 'i'}, {'n', n}, {'i', time}, {'m', m}, {'u', uid}, {'d', id}, {'w', w}, {'h', h}, {'c', cid}} | {{'t', 'i'}, {'s', 1}} | n - имя, cid - chat id, uid - user id, id - msg id, w, h -высота и ширина фото | m в бд должен быть в шифровке, а не BLOB 
| **img data** | **=**{{"t", 'pi'}, {'c', cid}, {'i', id}, {"np", np}, {"vp", vp}, {'p', m} } | {{'t', 'i'}, {'s', 1}} | cid - chat id, id - айди сообщения, np - номер текущего пакета, vp - количество пакетов, m - байты изображения в base64 | m в бд должен быть в шифровке, а не BLOB |
...
| **doc data** | **=**{{'t', 'di'}, {'i', id}, {'c', cid}, {"np", np}, {"vp", vp}, {'p', m} } | {{'t', 'd'}, {'i', did}} | cid - chat id, id - айди сообщения, np - номер текущего пакета, vp - количество пакетов, m - байты документа в base64 | сервер пересылает это сообщение без b, но с did |
...
| название | тип | для чего |
| - | - | - |
| id | string | айди сообщения cid_id |
| cid | int | айди чата |
| uid | int | userid |
